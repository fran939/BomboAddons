...
export class Persistence {
    static data = new PogObject("Bomboaddons", {
        inventoryButtons: {},
        commandAliases: {},
        keyShortcuts: {},
        messageTitles: {},
    ...
    static getDataFromFile(filePath, defaultValue = {}) {
        return JSON.parse(FileLib.read("Bomboaddons", `data/${filePath}`)) ?? defaultValue
    }
    ...
    static getDataFromFileOrLink(filePath, url, defaultValue = {}) {
        const thefile = FileLib.read("Bomboaddons", `data/${filePath}`)
    ...
    static saveDataToFile(filePath, data = {}, createFolderTree = true, indent = true) {
        FileLib.write("Bomboaddons", `data/${filePath}`, indent ? JSON.stringify(data, null, 4) : JSON.stringify(data), createFolderTree)
    }
